...
class DoctorFormViewModel: ViewModel() {
    ...
    var registrationId by mutableStateOf("")
    ...
    fun signUp(user: doctorInfo) {
        ...
        if(email.isNotEmpty() && registrationId .isNotEmpty()) {
            auth.createUserWithEmailAndPassword(user.email, user.registrationId)
                .addOnSuccessListener {
                    val newUser = hashMapOf(
                        "name" to user.name,
                        "email" to user.email,
                        "Registration Id" to user.registrationId,
    ...
    fun generateAndStoreUsername(name: String): String {
        ...
        val usersRef = firestore.collection("Doctor")
